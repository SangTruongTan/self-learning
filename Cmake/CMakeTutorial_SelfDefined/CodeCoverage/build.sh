...
BUILD="cmake --build $PROG_PATH/build --target CodeCoverage_coverage_gcovr CodeCoverage_coverage_lcov"
...
rsync -ah --progress $PROG_PATH/build/CodeCoverage_coverage_gcovr $PROG_PATH/coverages/
EXIT_CODE=$?
if [[ $EXIT_CODE != "0" ]] ; then
    return $EXIT_CODE
fi
rsync -ah --progress $PROG_PATH/build/CodeCoverage_coverage_lcov $PROG_PATH/coverages/
EXIT_CODE=$?
if [[ $EXIT_CODE != "0" ]] ; then
    return $EXIT_CODE
fi
