...
/* Defined periodic timer period in ticks. */
...
void vTask2(void *pvParameters) {
    ...
    xPeriodicTimer = xTimerCreate(
        "Periodic Timer", /* Text name to facilitate debugging. */
        TIMER_PERIOD,     /* The period for the timer. */
        pdTRUE,           /* Auto-reload - hence  a periodic timer. */
    ...
    /* Start the one periodic timer and check that it reports its state
     * correctly.
     */
    xTimerStart(xPeriodicTimer, (TickType_t)0);
    if (xTimerIsTimerActive(xPeriodicTimer) == pdFALSE) {
        configASSERT(pdFAIL);
    }
    /* As per most tasks, this task is implemented in an infinite loop. */
    for (;;) {
